...
function ConfidenceBadge({ score, isInScope, t }: { score: number; isInScope: boolean; t: typeof LIGHT }) {
  ...
  if (!isInScope && score < 0.45) {
    return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, padding: '4px 12px', borderRadius: 999, background: 'rgba(239,68,68,0.1)', color: '#f87171', border: '1px solid rgba(239,68,68,0.25)' }}>
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, padding: '4px 12px', borderRadius: 999, background: conf.b, color: conf.c, border: `1px solid ${conf.bd}` }}>
...
export default function Home() {
  ...
  async function handleSend() {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        similarityScore: data.similarity_score, isInScope: data.is_in_scope,
      }]);
    } catch {
  ...
  const Header = () => (
    <header style={{ position: 'sticky', top: 0, zIndex: 100, padding: '16px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${t.rule}`, background: theme === 'light' ? 'rgba(244,246,242,0.96)' : 'rgba(14,14,14,0.96)', backdropFilter: 'blur(12px)', transition: 'all 0.3s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <button className="theme-btn" onClick={() => setTheme(t => t === 'light' ? 'dark' : 'light')}
          style={{ width: 40, height: 40, borderRadius: '50%', border: `1.5px solid ${t.rule}`, background: t.surface, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: t.shadow }}>
  ...
  // Landing page — everything fits in one screen
  if (!doc) {
    return (
      <div className="doc-app" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: t.bg, color: t.ink, transition: 'background 0.3s, color 0.3s', overflow: 'hidden' }}>
        <style>{css(theme)}</style>
        <Header />

        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 660, margin: '0 auto', width: '100%', padding: '16px 24px' }}>

          {/* Heading */}
          <div style={{ textAlign: 'center', marginBottom: 16 }}>
            <h1 style={{ fontFamily: '"Fraunces", serif', fontWeight: 900, fontSize: 'clamp(2.2rem, 4.5vw, 3.8rem)', lineHeight: 0.95, letterSpacing: '-0.03em', color: t.ink, marginBottom: 12 }}>
              Ask anything about<br />
              <em style={{ fontStyle: 'italic', fontWeight: 300, color: t.accent }}>any document</em>
            </h1>
            <p style={{ fontSize: 16, fontWeight: 300, color: t.mid, lineHeight: 1.6, marginBottom: 14 }}>
              Upload a PDF and get instant answers in plain English.<br />
              No searching, no scrolling — just ask.
            </p>

            {/* Status pill */}
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: t.surface, border: `1px solid ${t.rule}`, borderRadius: 100, padding: '7px 16px', boxShadow: t.shadow }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: serverReady ? '#4ade80' : t.accent, display: 'inline-block' }} />
          ...
          <div
            ...
            style={{ border: `2px dashed ${isDragging ? t.accent : t.rule}`, borderRadius: 20, padding: '28px 32px', cursor: isUploading ? 'default' : 'pointer', background: isDragging ? t.accentLight : t.surface, marginBottom: 12, boxShadow: t.shadow, textAlign: 'center' }}
          ...
          >
            ...
            {isUploading ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 52, height: 52, borderRadius: 16, background: t.accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${t.rule}` }}>
                  <Loader2 size={26} color={t.accent} className="animate-spin" />
                </div>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, fontFamily: '"Fraunces", serif', color: t.ink, marginBottom: 4 }}>Reading your document...</div>
                  <div style={{ fontSize: 14, color: t.muted, fontWeight: 300 }}>Almost ready — this takes about 20 seconds</div>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 52, height: 52, borderRadius: 16, background: isDragging ? t.accent : t.accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${t.rule}`, transition: 'all 0.3s' }}>
                  <Upload size={26} color={isDragging ? t.accentInk : t.accent} />
                </div>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 700, fontFamily: '"Fraunces", serif', color: t.ink, marginBottom: 4 }}>Drop your PDF here</div>
                  <div style={{ fontSize: 14, color: t.muted, fontWeight: 300 }}>or click to browse files</div>
                </div>
                <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: t.muted, background: t.bg, padding: '4px 14px', borderRadius: 100, border: `1px solid ${t.rule}` }}>
                  PDF only
                </div>
              </div>
            )}
          ...
          {uploadError && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, color: '#ef4444', marginBottom: 10 }}>
              <AlertCircle size={15} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
            {[
              { icon: Upload,        num: '01', label: 'Upload', desc: 'Drop any PDF — reports, research, contracts, anything.' },
              ...
              <div key={step.num} className="how-card" style={{ background: t.surface, borderRadius: 14, padding: '16px 16px', border: `1px solid ${t.rule}`, boxShadow: t.shadow }}>
                <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: t.accent, marginBottom: 10 }}>{step.num}</div>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: t.accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 10, border: `1px solid ${t.rule}` }}>
                  <step.icon size={18} color={t.accent} />
                </div>
                <div style={{ fontSize: 15, fontWeight: 700, color: t.ink, marginBottom: 5, fontFamily: '"Fraunces", serif' }}>{step.label}</div>
                <div style={{ fontSize: 13, color: t.muted, lineHeight: 1.5, fontWeight: 300 }}>{step.desc}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          {messages.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 24, paddingTop: 64 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 16, color: t.muted, fontWeight: 300 }}>Ask anything about it below</div>
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center', marginTop: 8 }}>
                {['What is the main topic?', 'Summarize the key points', 'What are the conclusions?'].map((s) => (
                  <button key={s} className="chip" onClick={() => setInput(s)} style={{ fontSize: 14, fontWeight: 500, padding: '10px 18px', borderRadius: 100, background: t.surface, border: `1.5px solid ${t.rule}`, color: t.mid, cursor: 'pointer', boxShadow: t.shadow }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {messages.map((msg, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  <div style={{ maxWidth: '80%', borderRadius: msg.role === 'user' ? '20px 20px 4px 20px' : '20px 20px 20px 4px', padding: '14px 18px', background: msg.role === 'user' ? t.accent : t.surface, border: msg.role === 'user' ? 'none' : `1px solid ${t.rule}`, boxShadow: t.shadow }}>
                    {msg.role === 'user' ? (
